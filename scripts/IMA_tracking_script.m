...
RawDataDir = 'C:\Users\imadams\Documents\smite workspace\20250625_A549_ALFA-EGFR';
BeadsFileName = smi_helpers.getFileNames(RawDataDir, 'Bead*.mat');
BeadFile = fullfile(RawDataDir, BeadsFileName{1});
BgFileName = smi_helpers.getFileNames(RawDataDir, 'Background*.mat');
BgFile = fullfile(RawDataDir, BgFileName{1});

load(BeadFile, 'sequence');
BeadData = single(sequence);
load(BgFile, 'sequence');
...
FrameRate = 20; % Data Collection Frame Rate (1/s)
PixelSize = 0.1667; % Camera back-projected pixel size (micrometers)
...
SMFChannel1.Data.DataROI = [1, 1, 128, 128, 1, 1]; % [YStart, XStart, YEnd, XEnd, ZStart, ZPeriod]
...
SMFChannel1.BoxFinding.BoxSize = 7; % Linear box size for fitting (Pixels)
SMFChannel1.BoxFinding.BoxOverlap = 3; % Overlap of boxes allowed (Pixels)
SMFChannel1.BoxFinding.MinPhotons = 50;
SMFChannel1.Fitting.PSFSigma = 1.1811; % Initial or fixed Sigma of 2D Gaussian PSF Model (Pixels)
SMFChannel1.Fitting.FitType = 'XYNBS';
SMFChannel1.Thresholding.AutoThreshLogL = true;
SMFChannel1.Thresholding.MaxXY_SE = 0.2; % pixels
SMFChannel1.Thresholding.MinPhotons = 100; % Minimum accepted photons from fit
SMFChannel1.Thresholding.MinPSFSigma = 0.6; % Minimum PSF Sigma from fit (Pixels)
SMFChannel1.Thresholding.MaxPSFSigma = 2; % Maximum PSF Sigma from fit (Pixels)
SMFChannel1.Tracking.D = 0.01; % pixel^2/frame
SMFChannel1.Tracking.K_on = 0.9; % Off to On Rate (Frame^-1)
SMFChannel1.Tracking.K_off = 0.1; % On to Off Rate (Frame^-1)
SMFChannel1.Tracking.MaxDistFF = 5; % Maximum distance gap for frame-to-frame connection (Pixels)
SMFChannel1.Tracking.MaxDistGC = 5; % Maximum distance gap for Gap Closing (Pixels)
SMFChannel1.Tracking.MaxFrameGap = 20; % Maximum frame gap for Gap Closing (Pixels)
SMFChannel1.Tracking.MinTrackLength = 20; % Minimum track length of trajectory (Frames)
SMFChannel1.Tracking.TrajwiseD = true; % Max. number of batch tracking iterations (Integer)
SMFChannel1.Tracking.NIterMaxBatch = 10; % 
SMFChannel1.Tracking.NIterMax = 1; % Max. number of iterative tracking attempts (Integer) % maybe safer to use 1 and change NIterMaxBatch
% SMFChannel2 = copy(SMFChannel1);
% SMFChannel2.Data.AnalysisID = 'Channel2';
% SMFChannel2.Data.DataROI = [1, 129, 128, 256, 1, 1]; % [YStart, XStart, YEnd, XEnd, ZStart, ZPeriod]
% SMFChannel2.Tracking.D = 0.01; % pixel^2/frame
% SMFChannel2.Tracking.K_on = 0.9; % Off to On Rate (Frame^-1)
% SMFChannel2.Tracking.K_off = 0.1; % On to Off Rate (Frame^-1)
...
%
% [TR, SMD, SMDPreThresh] = performFullAnalysis(SPTChannel2);
